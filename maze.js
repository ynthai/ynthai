...
function requestVibrationPermission() {
    if ("vibrate" in navigator && !vibrationPermissionRequested) {
        ...
        alert("Для лучшего игрового опыта разрешите вибрацию.");
        // Здесь можно добавить более продвинутый UI для запроса разрешения,
        // например, модальное окно или попап.
    }
}
...
function vibrateDevice(duration) {
    if ("vibrate" in navigator) {
        if (navigator.userAgent.match(/iPhone|iPad|iPod/i)) {
            // Для iOS используем специальный паттерн
            navigator.vibrate([0, duration, 1]);
        } else {
            // Для других устройств просто вызываем vibrate
    ...
    move(dx, dy) {
        ...
        if ((dx === 1 && currentCell.walls[1]) ||
            ...
            if (!("vibrate" in navigator)) {
                ...
                vibrateDevice(100); // Вибрация 100 мс
            }
        }
        animatePlayer();
        checkWin();
