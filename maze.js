...
function checkWin() {
    if (maze.isExit(player.x, player.y)) {
        exitOpen = true;
        animateExit();
        if ("vibrate" in navigator) navigator.vibrate([100, 50, 100]); // Вибрация: 100 мс, пауза 50 мс, 100 мс
...
function checkNPCCollision() {
    npcs.forEach(npc => {
        if (npc.x === player.x && npc.y === player.y) {
            if ("vibrate" in navigator) navigator.vibrate([200, 100, 200]); // Вибрация: 200 мс, пауза 100 мс, 200 мс
